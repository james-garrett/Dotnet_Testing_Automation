...
namespace AutomationPractice.Core.DI
{
    ...
    public static class ContainerExtensions
    {
        public static IServiceCollection RegisterContainers(this IServiceCollection collection)
        {
            ...
            foreach (var container in Assembly.GetAssembly(type)!.GetTypes()
                .Where(t => t.IsClass && type.IsAssignableFrom(t)))
            {
                //Containers should be in Automation.Core.DI.Containers namespace
        ...
        public static IServiceCollection UseTestConfiguration<T>(this IServiceCollection container)
            where T : class, new()
        {
